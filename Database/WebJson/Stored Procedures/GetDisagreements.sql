...
BEGIN
  ...
  SET @jsonResult = 
  ( 
    SELECT wi.WorkId, wi.RowKey, wi.RawText AS SrcText, wi.CheckSrc,
      (
        SELECT tb.RawText, ul.UserName, tb.BlockId, tb.CheckRaw
        FROM dbo.TextBlockManual tb
        JOIN dbo.UserList ul ON ul.LogTo = tb.LogTo
        WHERE tb.ProjectId = @ProjectId AND tb.WorkId = wi.WorkId AND tb.LangKey = @LangKey
        FOR JSON PATH
      ) AS Candidate
    FROM Web.GetTextBlockDisagreements( @ProjectId, @LangKey) AS tbd
