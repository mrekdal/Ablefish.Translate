...
RETURN
(
    SELECT tb.WorkId, @LangKey AS LangKey, MIN(tb.CheckRaw) AS MinCheck, MAX(tb.CheckRaw) AS MaxCheck, MIN(tb.LogTo) AS MinLogTo, MAX(tb.LogTo) AS MaxLogTo, COUNT(*) AS n
    ...
    WHERE tb.LangKey = @LangKey AND tb.IsDiscarded = 0 -- Exclude discarded text blocks
    GROUP BY tb.WorkId
