CREATE PROCEDURE [Web].[GetWorkBatch]( @ProjectId INT, @LogTo VARCHAR(16), @LangWork VARCHAR(12), @LangHelp VARCHAR(12) = 'en', @LogToAi VARCHAR(12) = 'DeepL' ) AS
BEGIN
  SET NOCOUNT ON;
  ...
  LEFT JOIN dbo.TextBlock td ON td.WorkId = wi.WorkId AND td.CheckSrc = wi.CheckSrc AND td.LangKey = @LangWork AND td.LogTo = @LogToAi
  LEFT JOIN dbo.TextBlock tsh ON tsh.WorkId = wi.WorkId AND tsh.CheckSrc = wi.CheckSrc AND tsh.LangKey = @LangHelp AND tsh.LogTo = pth.GoldLogTo
  LEFT JOIN dbo.TextBlock tw ON tw.WorkId = wi.WorkId AND tw.CheckSrc = wi.CheckSrc AND tw.LangKey = @LangWork AND ( tw.LogTo = @LogTo OR tw.LogTo = @CoLogTo )
  ...
  WHERE wi.ProjectId = @ProjectId AND tw.BlockId IS NULL;
END