CREATE PROCEDURE [dbo].[AddTextBlockRowKey]( @ProjectId INT, @RowKey VARCHAR(64), @LangKey VARCHAR(12), @RawText NVARCHAR(MAX), @CheckSrc INT, @LogTo VARCHAR(16) ) AS
BEGIN
  ...
  WHEN MATCHED THEN
    UPDATE SET trg.RawText = @RawText, CheckSrc = @CheckSrc
  WHEN NOT MATCHED THEN
    INSERT ( WorkId, LangKey, RawText, CheckSrc, LogTo ) VALUES ( src.WorkId, @LangKey, @RawText, @CheckSrc, @LogTo );
